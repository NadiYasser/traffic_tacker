...
spark = ( SparkSession.builder
         .appName("TrafficSilverLakehouse")
...
dq_df = bronze_df.withColumn(
    "dq_flag",
    when(col("vehicule_id").isNull(),"MISSING_VEHICULE")
...
# Filter good records
clean_stream = validated.filter((col("dq_flag") == "OK") & (col("speed_valid")==1) & (col("time_valid")==1))

watermarked = clean_stream.withWatermark("event_ts","15 minutes")

deduped = watermarked.drop_duplicates(["vehicule_id","event_ts"])

silver_final = (
    deduped
    .withColumn("hour", hour("event_ts"))
    .withColumn("peak_flag",when((col("hour").between(8,11)) | (col("hour").between(8,11)) ,1 ).otherwise(0))
